...
<div class="m-subheader ">
    <div class="d-flex align-items-center">
        <div class="mr-auto">
            <h3 class="m-subheader__title ">Nhóm Danh Mục</h3>
        </div>
    </div>
</div>
<!-- END: Subheader -->
						...
						<div class="m-dropdown m-dropdown--inline m-dropdown--arrow m-dropdown--align-right m-dropdown--align-push" m-dropdown-toggle="hover" aria-expanded="true">
							<a href="{{route('category.create')}}" class="btn m-btn m-btn--gradient-from-primary m-btn--gradient-to-info">
								<span>
									...
									<span>Tạo Nhóm Danh Mục Mới</span>
								</span>
							</a>
						</div>
					</li>
		...
		<div class="m-portlet__body">
				...
				<form class="form-group m-form__group" method="GET">
					...
					<div class="m-portlet__foot m-portlet__foot--fit">
						<div class="m--margin-top-15">
							<button type="submit" class="btn m-btn--square  m-btn m-btn--gradient-from-success m-btn--gradient-to-accent">Tìm Kiếm</button>
			...
			<div class="m_datatable m-datatable m-datatable--default m-datatable--loaded" id="local_data" style="">
				<table class="m-datatable__table" style="display: block; min-height: 300px; overflow-x: auto;">
					<thead class="m-datatable__head">
						<tr class="m-datatable__row">
                            ...
                            <th data-field="ShipName" class="m-datatable__cell m-datatable__cell--sort">
								<span style="width: 130px;">Danh Mục Con</span>
							</th>
					...
					<tbody class="m-datatable__body" style="">
						@foreach($parentCategory as $prCategory)
						<tr class="m-datatable__row m-datatable__row--even" style="left: 0px;">
							<td data-field="OrderID" class="m-datatable__cell">
								<span style="width: 110px;">{{(($parentCategory->currentPage()-1)*5) + $loop->iteration}}</span>
							</td>
							<td data-field="ShipName" class="m-datatable__cell">
								<span style="width: 110px;">{{$prCategory->name}}</span>
							</td>
                            <td data-field="ShipName" class="m-datatable__cell">
								<span style="width: 110px;" class="text-center">
                                    <span class="m-badge @if(count($prCategory->categories) > 0) m-badge--brand @else m-badge--secondary @endif m-badge--wide">{{count($prCategory->categories)}}</span>
								</span>
							</td>
							<td data-field="Actions" class="m-datatable__cell">
								<span style="overflow: visible; position: relative; width: 110px;">
                                    <a class="m-portlet__nav-link btn m-btn m-btn--hover-danger m-btn--icon m-btn--icon-only m-btn--pill" href="javascript:;">
                                        <i class="la la-trash" onclick="return confirm('Nhóm Danh Mục Này Đang Hoạt Động, Ko Thể Xóa!')"></i>
                                    </a>
									<a href="{{route('categoryGroup.edit', ['id' => $prCategory->id])}}" class="m-portlet__nav-link btn m-btn m-btn--hover-accent m-btn--icon m-btn--icon-only m-btn--pill" title="Sửa ">
				...
				<div class="m-datatable__pager m-datatable--paging-loaded clearfix">
					{{ $parentCategory->links('vendor.pagination.custom') }}
				</div>
			</div>
