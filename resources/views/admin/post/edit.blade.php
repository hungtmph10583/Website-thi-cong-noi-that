...
<div class="m-subheader ">
    <div class="d-flex align-items-center">
        <div class="mr-auto">
            <h3 class="m-subheader__title m-subheader__title--separator">Sửa Bài Viết</h3>
			...
			<div class="m-portlet__body" data-select2-id="82">
				<div class="form-group m-form__group row">
					...
					<div class="col-lg-4 col-md-9 col-sm-12">
                        <input type="text" name="title" class="form-control" value="{{$post->title}}" placeholder="Tên Bài Viết">
                        @error('title')
					...
					<div class="col-lg-4 col-md-9 col-sm-12">
                        <div class="m-radio-inline">
							<label class="m-radio m-radio--solid">
                                <input type="radio" name="status" value="1"{{ ($post->status == 1 ? 'checked' : '') }}> Hiển Thị
                                <span></span>
                            </label>
                            <label class="m-radio m-radio--solid">
                                <input type="radio" name="status" value="2"{{ ($post->status == 2 ? 'checked' : '') }}> Không HIển Thị
                                <span></span>
                            </label>
						</div>
					</div>
				...
				<div class="form-group m-form__group row">
					...
					<div class="col-lg-4 col-md-9 col-sm-12">
						<textarea class="form-control m-input m-input--square" name="description" rows="4">{{$post->description}}</textarea>
                        @error('description')
                            <span class="m-form__help m--font-danger">{{$message}}</span>
                        @enderror
					</div>
				...
				<div class="form-group m-form__group">
					<label class="">Nội Dung Bài Viết</label>
						<textarea class="form-control m-input m-input--square" name="content" id="ckeditor" rows="8">{{$post->content}}</textarea>
                        @error('content')
                            <span class="m-form__help m--font-danger">{{$message}}</span>
                        @enderror
				</div>
...
<script type="texT/javascript">
    ...
    const nodeList= document.querySelectorAll("p > span > img");
